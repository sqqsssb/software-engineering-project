...
from test_phase_control import TestPhase
...
def run_prompt():
    with run_lock:  # 加锁，确保同一时间只有一个请求执行
        ...
        request_count += 1
        print(
            f"收到第 {request_count} 次请求，Prompt: {request.form.get('prompt')}, Name: {request.form.get('name')}, Model: {request.form.get('model')}, Path: {request.form.get('path')}, Config: {request.form.get('config')}, Org: {request.form.get('org')}")
